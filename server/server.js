...
// Modify the attendance API endpoint to better handle check-in/check-out
// Find the app.post("/api/attendance") route and replace it with:
...
app.post("/api/attendance", authenticateToken, async (req, res) => {
  try {
    ...
    const existingRecord = await pool.query("SELECT * FROM attendance WHERE employee_id = $1 AND date = $2", [
      employee_id,
      date,
    ])
