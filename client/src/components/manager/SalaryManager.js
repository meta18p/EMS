...
//       const res = await axios.post("http://localhost:5000/api/salary/calculate", {
//         month: selectedMonth + 1,
//         year: selectedYear,
//       })
...
//                         className="text-blue-500 hover:text-blue-700"
//                         onClick={() => {
//                           // Emit socket event to notify employee about salary update
//                           if (socket && connected) {
//                             socket.emit("salary_update", {
//                               employee_id: employee.id,
//                               month: selectedMonth + 1,
...
//     // Calculate overtime (assuming 10% bonus for each day marked as overtime)
//     const overtime = monthAttendance.reduce((total, record) => {
//       if (record.overtime_hours > 0) {
//         return total + (baseSalary / 30) * 0.1 * record.overtime_hours
//       }
//       return total
//     }, 0)
...
//                     <td className="py-3 px-4">${Number(salary.baseSalary).toFixed(2)}</td>
//                     <td className="py-3 px-4 text-green-600">+${salary.overtime.toFixed(2)}</td>
//                     <td className="py-3 px-4 text-red-600">-${salary.unpaidLeaveDeduction.toFixed(2)}</td>
//                     <td className="py-3 px-4 font-bold">${salary.finalSalary.toFixed(2)}</td>
//                   </tr>
//                 )
//               })
